...
There is growing interest in characterizing wildlife distributions across the full-annual cycle, as opposed to one or two seasons, so that we can better understand the places and times at which populations are especially vulnerable. Maps play a central role in the process of acquiring and representing knowledge. In the past, range maps for resident species typically contained a single block of color along with political boundaries that indicated where a species can be found throughout the year. For migratory species, range maps contained several blocks of color indicating where a species might be found during a particular season, or throughout the entire year. 
...
```{r fisher-distill}
# distill distributional information across layers
m2_distill <- metrics_distill(fisher_ud)

# visualize all layers on a single map
map_single(m2_distill, p2, lambda = -5)
```
...
### Example 3: Map distributions of multiple individuals during the same time period
...
# visualize both distributions on a single map
...
Despite the general similarity of the two utilization distributions, some places were used almost exclusively by LA11 (i.e., blue cells) and others were used almost exclusively by LA14 (i.e., red cells). We can contextualize these patterns by adding information about the locations of pans, waterholes, roads, and boundaries to the map. First we need to load relevant spatial data and make sure that the data sets are described by the same coordinate reference system as the elephant data. This contextual information can be [downloaded as a GeoPackage](https://github.com/mstrimas/colorist/raw/master/data-raw/etosha-features.gpkg) from the package GitHub repository, which we do below.
...
Now these layers can be loaded into R from the GeoPackage.

```{r elepaphant-sf, eval = nzchar(Sys.getenv("BUILD_VIGNETTES"))}
...
```{r elephant-pretty, dpi = 600, fig.width = 6, fig.height = 3.5, eval = nzchar(Sys.getenv("BUILD_VIGNETTES"))}
