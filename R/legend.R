...
#'   of intensity values. "Maximum intensity" is the default.
#' @param label_l character vector with a single element describing the meaning
#'   of layer values. "Layer" is the default.
#' @param label_s character vector with three elements describing differences
#'   in the meaning of three specificity values (i.e., 0, 50, 100). "Low specificity",
#'   "Moderate specificity", and "High specificity" are the default.
...
legend_timecycle <- function(palette, specificity = TRUE, origin_label = NULL,
                             label_i = NULL, label_l = NULL, label_s = NULL,
                             return_df = FALSE) {
  ...
  # make intensity label for legend
  if (!is.null(label_i)) {
    labi <- label_i
  } else {
    labi <- "Maximum\nintensity"
...
#'   of layer values. "Layer" is the default.
#' @param label_s character vector with three elements describing differences
#'   in the meaning of specificity across three legend wheels. "Low specificity",
#'   "Moderate specificity", and "High specificity" are the default.
#' @param return_df logical indicating whether to return the legend as a
#'   `ggplot2` object or return a data frame containing the necessary data to
#'   build the legend.
...
legend_timeline <- function(palette, specificity = TRUE, time_labels = NULL,
                            label_i = NULL, label_l = NULL, label_s = NULL, axis_i = NULL,
                            return_df = FALSE) {
  ...
  # make intensity axis labels for legend
  if (!is.null(axis_i)) {
    axi <- axis_i
  } else {
    axi <- c("low", "high")
...
#'   low and high specificity values. "low" and "high" are the default.
...
legend_set <- function(palette, specificity = TRUE, group_labels = NULL,
                       label_i = NULL, label_s = NULL, axis_i = NULL,
                       axis_s = NULL, return_df = FALSE) {
  ...
  # make intensity axis label for legend
  if (!is.null(axis_i)) {
    axi <- axis_i
  } else {
    axi <- c("low", "high")
  ...
  p <- ggplot2::ggplot(data = wheel) +
    ...
    ggplot2::theme(plot.background = ggplot2::element_rect(fill = "white"),
                   ...
                   axis.text.x = ggplot2::element_text(size = 9, hjust = 0.5, vjust = 0.5),
                   axis.text.y = ggplot2::element_text(size = 9, hjust = 0.5, vjust = 0.5, angle = 90)) +
    ggplot2::scale_x_continuous(breaks = c(15, 85), labels = axs) +
    ggplot2::scale_y_continuous(breaks = c(0.15, 0.85), labels = axi) +
    ggplot2::xlab(labs) +
    ggplot2::ylab(labi) +
